...
crash_data %>% filter(contributing_factor_vehicle_1 == "Unsafe Speed" | contributing_factor_vehicle_2 == "Unsafe Speed") %>% 
  summarize(killed = sum(number_of_persons_killed),
...
Stat for blog
```{r}
street_sum %>% as.data.frame() %>%  filter(prop_25 >= .7) %>% summarize(crashes = sum(unsafe_speed_crashes),
                                                                        tcrashes = sum(total_crashes))

street_sum %>% as.data.frame() %>%  filter(prop_25 >= .7) %>%
...
write_csv(top50, "top50_speedcrashpmile.csv")
...
title <- tags$div(
  tag.map.title, HTML("<h2>NYC's dangerous 25mph streets</h2><br><p>25 mph streets with the most collisions per mile caused by unsafe speed<br><span>Select a street</span> to view the number of collisions since 2019</p>")
)  
